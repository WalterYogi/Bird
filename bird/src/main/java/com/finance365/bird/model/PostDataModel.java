 ...
 */
public class PostDataModel implements InterfaceModel.IObjectModel{
    @Override
    public void getData(RequestUrl requestUrl, final StringCallback callback) {
        OkHttpUtils.post().url(requestUrl.url).params(requestUrl.getParams()).build().execute(new Callback() {
            @Override
