package com.finance365.bird.ui.view;
...
public class SegmentedGroup extends RadioGroup {
    ...
    private int mUnCheckTextColor = Color.WHITE;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        try {
            mMarginDp = (int) typedArray.getDimension(R.styleable.SegmentedGroup_sc_border_width, getResources().getDimension(R.dimen.radio_button_stroke_border));
            mCornerRadius = typedArray.getDimension(R.styleable.SegmentedGroup_sc_corner_radius, getResources().getDimension(R.dimen.radio_button_conner_radius));
            mTintColor = typedArray.getColor(R.styleable.SegmentedGroup_sc_tint_color, getResources().getColor(R.color.radio_button_selected_color));
            mUnCheckTextColor = typedArray.getColor(R.styleable.SegmentedGroup_sc_unchecked_text_color, getResources().getColor(R.color.radio_button_selected_color));
            mCheckedTextColor = typedArray.getColor(R.styleable.SegmentedGroup_sc_checked_text_color, getResources().getColor(android.R.color.white));
        } finally {
            typedArray.recycle();
        }
    }
    ...
    public void setTintColor(int tintColor, int checkedTextColor, int unCheckTextColor) {
        mTintColor = tintColor;
        mCheckedTextColor = checkedTextColor;
        mUnCheckTextColor = unCheckTextColor;
        updateBackground();
    }
    ...
    private void updateBackground(View view) {
        ...
        ColorStateList colorStateList = new ColorStateList(new int[][]{
                ...
                {-android.R.attr.state_pressed, android.R.attr.state_checked}},
                new int[]{Color.GRAY, mUnCheckTextColor, mCheckedTextColor});
        ((Button) view).setTextColor(colorStateList);

        //Redraw with tint color
        Drawable checkedDrawable = ContextCompat.getDrawable(context, checked).mutate();
        Drawable uncheckedDrawable = ContextCompat.getDrawable(context, unchecked).mutate();
